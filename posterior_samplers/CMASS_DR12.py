...
class CMASSPosterior:
    ...
    def __init__(self, data_directory, smooth=10., rmax=120., chain_number=1, like='SH',
        ...
        :param smooth:          float, optional
                                smoothing scale used for reconstruction
        ...
        self.fiducial_b = 1.85  # nominal bias value for CMASS galaxies (from Alam et al. 2017)
        ...
        if mono_from_mocks:
            # unfortunately, for the mocks we only have this grid for smoothing scale 10 Mpc/h due to the massive
            # computational effort required to compute it; so if using smooth=15 you just have to hope or pretend that
            # the mean mock monopole will not change significantly for the different smoothing scales (or use the data
            # monopole option, but this can lead to correlations between data and theory model that result in small
        ...
        self.mc_output_file = 'CMASS_Rs%0.1f_emcee%d_%s%s%s%s.npy' % (smooth, chain_number, likestring,
        ...
        # output destination
        self.ml_output_file = 'CMASS_Rs%0.1f_ML_%s%s%s%s.txt' % (smooth, likestring, monostring, covstring, multstring)
        # ------------------------------------------------------------- #
    ...
    def find_max_like(self):
        ...
        # we fix alpha = 1.0 by hand here because it does not affect the posterior at all
    ...
    def theory_multipoles(self, fs8, bs8, sigma_v, alpha_perp, alpha_par, s, mu, rescale_all=True):
        ...
        # we therefore determine f from beta using fixed b=1.85
        ...
        try:
            for i in range(len(data_p_multipoles)):
                ...
                data_p_multipoles[i] = interpolater(beta * self.fiducial_b)
        except ValueError:
    ...
    def lnlike(self, theta):
        ...
        try:
            for i in range(len(data_s_multipoles)):
                ...
                data_s_multipoles[i] = interpolater(beta * self.fiducial_b)
        except ValueError:
    ...
    # def gaussian_proposal(self, theta, step_k, update_method='global'):
    #     """
    #     Propose step in parameter space starting from given location
    #
    #     :param theta: array, length N
    #                   current location in parameter space
    #     :param step_k: integer
    #                    current step number in the chain (used in sequential updating of parameters)
    #     :param update_method: string, 'global' or 'sequential'
    #                           sets choice of how to update parameters, default='global'
    #     :return: array of length N with new location in parameter space
    #     """
    #
    ...
    #             sigmas[i] = np.sqrt(1 / self.eigval[i] / len(theta)) * rd.gauss(0, 1)
    ...
    #     new_theta = theta + np.dot(self.eigvec, sigmas)
    #
    #     return new_theta

    # def run_metropolis(self, location):
    #     """
    #     Rudimentary routine for MCMC exploration of posterior using the Metropolis-Hastings sampling
    #
    #     :param location:        numpy array, length N
    #                             location in parameter space around which to start chain
    #     :return:
    #     """
    #
    #     # generate a starting point by taking random step away from given location
    #     start_location = self.gaussian_proposal(location, step_k=1, update_method='global')
    #     logpost = self.lnpost(start_location)
    #     i = 1
    #     while not(np.isfinite(logpost)) and i < 100:
    #         # try a new position
    #         start_location = self.gaussian_proposal(location, step_k=1, update_method='global')
    #         logpost = self.lnpost(start_location)
    #         i += 1
    #     if i == 100:
    #         print("Failed to find a good starting location after 100 tries, check proposal!")
    #         return
    #
    #     # keep track of acceptance fraction: first step has just been de facto accepted
    #     accepted, rejected, total = 1.0, 0.0, 1
    #
    #     # run the chain
    ...
    #     while continue_run:
    #
    #         chain = np.zeros((self.nstep, self.ndim + 1 + 3))   # store [-logpost, params, derived params]
    #
    #         # start by setting the 0th element of chain
    #         chain[0, 0] = -1.0 * logpost
    #         chain[0, 1:self.ndim + 1] = current_location
    #         apar = current_location[3] * (current_location[4]**(-2./3))
    #         aperp = current_location[4] * apar
    #         beta = current_location[0] / current_location[1]
    #         chain[0, self.ndim + 1:] = np.array([beta, aperp, apar])
    #         k = 1
    #
    #         while k < self.nstep:
    #
    #             # take a step in parameter space
    #             new_posn = self.gaussian_proposal(chain[k-1, 1:self.ndim + 1], total, update_method=self.update_method)
    #             new_logpost = self.lnpost(new_posn)
    #
    #             if not np.isfinite(new_logpost):
    ...
    #             if new_logpost >= logpost:
    #                 alpha = 1
    #             else:
    #                 alpha = np.exp(new_logpost - logpost)
    #
    #             if (alpha == 1.) or (rd.uniform(0, 1) < alpha):  # accept the step
    #                 # record new position in the chain
    #                 chain[k, 0] = -1.0 * new_logpost
    #                 chain[k, 1:self.ndim + 1] = new_posn
    #                 apar = new_posn[3] * (new_posn[4] ** (-2. / 3))
    #                 aperp = new_posn[4] * apar
    #                 beta = new_posn[0] / new_posn[1]
    #                 chain[k, self.ndim + 1:] = np.array([beta, aperp, apar])
    #
    ...
    #             k += 1
    #
    #         # at the end of self.nstep steps, write to file
    #         if icount == 0:
    #             full_chain = chain
    #             icount = 1
    #         else:
    #             full_chain = np.load(self.data_directory + 'MCMC_chains/' + self.mc_output_file, allow_pickle=True)
    #             full_chain = np.vstack([full_chain, chain])
    #         np.save(self.data_directory + 'MCMC_chains/' + self.mc_output_file, full_chain)
    #
    #         print("%d steps, current acceptance rate = %0.2f" % (total, accepted / total))
    #
    #         # now calculate the integrated autocorrelation time for params and decide whether to proceed
    #         full_chain = full_chain[:, 1:].reshape((full_chain.shape[0], 1, full_chain.shape[1] - 1))
    #         print("autocorrelation times for parameters:")
    #         acor = np.empty(self.ndim + 3)
    #         for i in range(acor.shape[0]):
    #             acor[i] = UtilMethods.autocorrelation(full_chain[:, :, i].T)
    #             print("\t %s: %0.3f" % (self.paramnames[i], acor[i]))
    #         sys.stdout.flush()
    #         if np.max(acor[[0, 4, 5, 6, 7]]) < self.stop_factor * total:
    #             # chain is long enough for convergence for parameters we care about
    #             print('Chain length exceeds %d x the maximum autocorrelation length for important params, so stopping'
    #                   % (1. / self.stop_factor))
    #             continue_run = False
    #         else:
    #             print('Chain length = %d, worst autocorrelation length = %d; continuing'
    #                   % (total, np.max(acor[[0, 4, 5, 6, 7]])))
    #         sys.stdout.flush()
